...
function previewImage(el,widths,limit){
	...
	reader.onload = (function(aImg) {
		return function(e) {
			...
			var description = document.createElement('p');
			description.innerHTML = '<br />Formato: <b>'+format+'</b> Tamaño: <b>'+(e.total/1024).toFixed(2)+'</b> KB.';
			...
			if (allowedTypes.indexOf(format)>=0 && e.total<(limit*1024*1024)){
				...
				description.innerHTML += '<br /><span style="color:green;">Imagen lista para subir.</span>';
			} else {
			    description.innerHTML += '<br /><span style="color:red;">El formato de imagen no es correcto - Formatos compatibles: '+allowedTypes.join(', ')+'. Tamaño máximo: '+(limit * 1024)+'KB</span>';
			}						
